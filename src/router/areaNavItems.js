const areaNavItems = [
  {name:"Animation", url:"/tag/animation"},
  {name:"Audiovisual", url:"/tag/audiovisual"},
  {name:"Design", url:"/tag/design"},
  {name:"Drawing", url:"/tag/drawing"},
  {name:"Film", url:"/tag/film"},
  {name:"Graphic Design", url:"/tag/graphic-design"},
  {name:"Installation", url:"/tag/installation"},
  {name:"Interactive", url:"/tag/interactive"},
  {name:"Motion Graphics", url:"/tag/motion-graphics"},
  {name:"Painting", url:"/tag/painting"},
  {name:"Performance", url:"/tag/performance"},
  {name:"Photography", url:"/tag/photography"},
  {name:"Printmaking", url:"/tag/printmaking"},
  {name:"Sculpture", url:"/tag/aculpture"},
  {name:"Sound Art", url:"/tag/sound-art"},
  {name:"Video", url:"/tag/video"},
  {name:"Website", url:"/tag/website"}
]

export default areaNavItems